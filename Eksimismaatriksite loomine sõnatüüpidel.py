...
jar = []
for i in range(len(list(ds2.columns)) - 1):
    x = pd.DataFrame(ds2.iloc[:, 0])
    y = pd.DataFrame(ds2.iloc[:, i + 1])
    jar.append((x, y))
...
models = [
    ...
    (("DECISION TREE CLASSIFIER", DecisionTreeClassifier(max_depth=16, min_samples_split=20))),
    (("RANDOM FOREST CLASSIFIER", RandomForestClassifier(max_depth=16, min_samples_split=20))),
    ...
    (("MLPCLASSIFIER", MLPClassifier(activation="logistic", alpha=1, hidden_layer_sizes=(8), learning_rate="invscaling", solver="adam", max_iter=1500)))
]
